...
// Telegram'dagi barcha guruhlarni avtomatik qo'shish
router.post('/add-all-from-telegram', async (req, res) => {
  try {
    // Telegram client olish
    let client;
    if (isServerMode) {
    ...
    for (const dialog of groups) {
      try {
        // Chat ID ni to'g'ri formatga aylantirish
        ...
        if (existing) {
          ...
          // Agar aktiv emas bo'lsa, aktivlashtirish
          if (Number(existing.active) !== 1) {
            await updateGroup(existing.id, { ...existing, active: true });
            activated++;
            console.log(`✓ Aktivlashtirildi: ${name}`);
          }
        } else {
    ...
    console.error('Add all groups error:', error);
    res.json({
      success: false,
