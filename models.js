...
async function savePhone(phone, groupId, messageText = '') {
  ...
  if (existing) {
    await query(
      `UPDATE phones SET last_date = datetime('now', 'localtime'), repeat_count = repeat_count + 1, last_message = ? WHERE id = ?`,
  ...
  try {
    ...
    const groupStats = groups.map(group => {
      ...
      const successRate = smsStat.sms_sent > 0
        ? ((Number(smsStat.sms_success) / Number(smsStat.sms_sent)) * 100).toFixed(1)
        : 0;
